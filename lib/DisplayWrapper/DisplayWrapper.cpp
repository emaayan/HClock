#include <DisplayWrapper.h>
#include <DebugUtils.h>

void DisplayWrapper::init()
{
    _lcd.init();
    _lcd.clear();
}

void DisplayWrapper::lightUp()
{
    _lcd.backlight();
}

void DisplayWrapper::clearRow(const uint8_t row, const bool rtl)
{
    char blank[_cols] = "";
    memset(blank, ' ', sizeof(blank));
    display(blank, 0, row, rtl);
}
void DisplayWrapper::println(const uint8_t row, const bool rtl, const char *fmt, ...)
{
    const size_t size = (rtl ? _cols * 2 : _cols) + 1;
    char buffer[size] = "";
    if (rtl)
    { // must be done cause don't know how to pad spaces
        clearRow(row, rtl);
    }

    va_list argptr;
    va_start(argptr, fmt);

    vsnprintf(buffer, sizeof(buffer), fmt, argptr);
    va_end(argptr);
    // debug("%d ",res);

    display(buffer, 0, row, rtl);
}

void DisplayWrapper::display(const char *buffer, const uint8_t col, const uint8_t row, const bool rtl)
{
    if (rtl)
    {
        _lcd.rightToLeft();
        _lcd.setCursor(_cols - (col + 1), row);
    }
    else
    {
        _lcd.leftToRight();
        _lcd.setCursor(col, row);
    }
    _lcd.print(buffer);
}
void DisplayWrapper::clear()
{
    _lcd.clear();
}
